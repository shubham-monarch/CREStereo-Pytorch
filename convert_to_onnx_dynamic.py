...
if __name__ == '__main__':
	...
	# defiune dynamic axes
	dynamic_axes = {'left': {0: 'batch_size'}, 'right': {0: 'batch_size'}, 'flow_init': {0: 'batch_size'}, 'output': {0: 'batch_size'}}
	
	# Export the model
	torch.onnx.export(model,               
	                  (t1, t2, flow_init),
	                  "crestereo_dynamic.onnx",   # where to save the model (can be a file or file-like object)
	...
	dynamic_axes_without_flow = {'left': {0: 'batch_size'}, 'right': {0: 'batch_size'}, 'output': {0: 'batch_size'}}

	torch.onnx.export(model,               
	                  (t1_half, t2_half),
	                  "crestereo_without_flow_dynamic.onnx",   # where to save the model (can be a file or file-like object)
	                  export_params=True,        # store the trained parameter weights inside the model file
	                  opset_version=12,          # the ONNX version to export the model to
	                  do_constant_folding=True,  # whether to execute constant folding for optimization
	                  input_names = ['left', 'right'],   # the model's input names
	                  output_names = ['output'],
					  dynamic_axes=dynamic_axes_without_flow)
